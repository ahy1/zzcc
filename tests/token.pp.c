...
struct pattern_s {
 int type;
 char *text;
 int token_type, token_subtype, token_encoding;
 int state;
};
# 147 "token.c"
static int last_cno;
...
struct token_s *gettoken(FILE *infp, STRBUF *sb, int *lno, int *cno)
{
 ...
 if (!(token=(struct token_s *)calloc(1, sizeof *token))) return ((void *)0);
 ...
 if ((ch=fetch(infp, lno, cno))==(-1)) return ((void *)0);
 ...
 case '\f':
  ...
  while ((((__ctype_ptr__+sizeof(""[(ch=fetch(infp, lno, cno))]))[(int)((ch=fetch(infp, lno, cno)))])&010)) {
   if (ch=='\n') token->subtype=WTT_NEWLINEWS;
   (void)sbput(token->sb, ch);
  }
 ...
 case 'n': case 'o': case 'p': case 'q': case 'r': case 's': case 't': case 'u': case 'v': case 'w': case 'x': case 'y': case 'z':
  ...
  (void)sbput(token->sb, ch);
  while ((((__ctype_ptr__+sizeof(""[(ch=fetch(infp, lno, cno))]))[(int)((ch=fetch(infp, lno, cno)))])&(01|02|04)) || ch=='_') {(void)sbput(token->sb, ch);}
  ...
  else if (!strcmp(sbcstr(token->sb, token->sbix), "__inline")) token->type=TT_INLINE;
  else if (!strcmp(sbcstr(token->sb, token->sbix), "__inline__")) token->type=TT_INLINE;
  else if (!strcmp(sbcstr(token->sb, token->sbix), "restrict")) token->type=TT_RESTRICT;
  else if (!strcmp(sbcstr(token->sb, token->sbix), "__restrict")) token->type=TT_RESTRICT;
 ...
 case '0':
  ...
  if ((ch=fetch(infp, lno, cno))=='x' || ch=='X') {
   ...
   (void)sbput(token->sb, ch);
   while((((__ctype_ptr__+sizeof(""[(ch=fetch(infp, lno, cno))]))[(int)((ch=fetch(infp, lno, cno)))])&(0100|04))) {sbput(token->sb, ch);}
  } else {
   token->encoding=NTE_OCTAL;
   if ((((__ctype_ptr__+sizeof(""[ch]))[(int)(ch)])&04)) {
    (void)sbput(token->sb, ch);
    while((((__ctype_ptr__+sizeof(""[(ch=fetch(infp, lno, cno))]))[(int)((ch=fetch(infp, lno, cno)))])&04)) {sbput(token->sb, ch);}
   }
  }
 ...
 case '1': case '2': case '3': case '4': case '5': case '6': case '7': case '8': case '9':
  ...
  token->type=TT_NUMBER;
  (void)sbput(token->sb, ch);
  while ((((__ctype_ptr__+sizeof(""[(ch=fetch(infp, lno, cno))]))[(int)((ch=fetch(infp, lno, cno)))])&04)) {(void)sbput(token->sb, ch);}
  if (ch=='.') {
   (void)sbput(token->sb, ch);
   while ((((__ctype_ptr__+sizeof(""[(ch=fetch(infp, lno, cno))]))[(int)((ch=fetch(infp, lno, cno)))])&04)) {(void)sbput(token->sb, ch);}
 ...
 case '/':
  ...
  if ((ch=fetch(infp, lno, cno))=='=') {
   ...
   (void)sbput(token->sb, ch);
   while ((ch=fetch(infp, lno, cno))!=(-1) && ch!='\n') {(void)sbput(token->sb, ch);}
   (void)sbput(token->sb, ch);
  } else if (ch=='*') {
   token->type=TT_WHITESPACE;
   ...
   (void)sbput(token->sb, ch);
   while (1) {
    while ((ch=fetch(infp, lno, cno))!=(-1) && ch!='*') {(void)sbput(token->sb, ch);}
    (void)sbput(token->sb, ch);
