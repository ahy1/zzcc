...
# 54 "/usr/lib/gcc/x86_64-pc-cygwin/4.8.3/include-fixed/sys/cdefs.h" 3 4
# 1 "/usr/lib/gcc/x86_64-pc-cygwin/4.8.3/include/stddef.h" 1 3 4
...
struct node_s {
 ...
 struct node_s *scope_parent;
 int type;
 int subtype;
 int level;
 int in_typedef_declaration;
...
# 1 "/usr/lib/gcc/x86_64-pc-cygwin/4.8.3/include/stdarg.h" 1 3 4
# 98 "/usr/lib/gcc/x86_64-pc-cygwin/4.8.3/include/stdarg.h" 3 4
...
struct node_s *g_root_node=((void *)0);

static void print_node_type(FILE *fp, struct node_s *node)
{
 fprintf(((__getreent())->_stderr), "{%-26s}", node_type_names[node->type]);
}
...
static void log_node_token(struct node_s *node, struct token_s *token, const char *fmt, ...)
{
 ...
 fprintf(((__getreent())->_stderr), "%02d %3d/%2d [%-8s] %-25s", level, token_lno(token), token_cno(token),
  token_text(token), token_type(token));

 while(level--) (void)putc(' ', ((__getreent())->_stderr));

 print_node_type(((__getreent())->_stderr), node);

 __builtin_va_start(arg,fmt);
 (void)vfprintf(((__getreent())->_stderr), fmt, arg);
...
struct nodeset_s {
 struct node_s nodes[16];
 size_t nnodes;
};

STACK *nodesets=((void *)0);
...
static void nodefree(struct node_s *node)
{
 ...
 if (nodeset->nnodes > 0u) --nodeset->nnodes;
...
static size_t error_node_token(struct node_s *node, struct token_s *token, const char *fmt, ...)
{
 ...
 fprintf(((__getreent())->_stderr), "ERROR (%s %d/%d %s %s): ",
...
static void indent(int ind)
{
 ...
 for (ix=0; ix<ind; ++ix) (void)putc(' ', ((__getreent())->_stdout));
}
...
void print_node_json(struct node_s *node, int ind)
{
 ...
 indent(ind);
 fputs("[{\"type\": ", ((__getreent())->_stdout));
 fputs(json_str(node_type_names[node->type], buf, 1024*1024), ((__getreent())->_stdout));
 fputs(", \"token\": {\"type\": ", ((__getreent())->_stdout));
 fputs(json_str(token_type(node->token), buf, 1024*1024), ((__getreent())->_stdout));
 fputs(", \"text\": ", ((__getreent())->_stdout));
 fputs(json_str(token_text(node->token), buf, 1024*1024), ((__getreent())->_stdout));
 fputs(", \"fname\": ", ((__getreent())->_stdout));
 fputs(json_str(token_fname(node->token), buf, 1024*1024), ((__getreent())->_stdout));
 ...
 int tt_separator, int opt_end_separator)
{
 ...
 fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("1"), (int)ix, token_text(tokens[ix]));

 if ((parsed=pf(node, tokens+ix))) ix+=parsed;
 else return free_node(node);

 fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("2"), (int)ix, token_text(tokens[ix]));
 while (tokens[ix]->type==tt_separator) {
  ++ix;
  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("2a"), (int)ix, token_text(tokens[ix]));
  ...
  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("2b"), (int)ix, token_text(tokens[ix]));
 }

 fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("3"), (int)ix, token_text(tokens[ix]));
 ...
 size_t (*pf)(struct node_s *, struct token_s **), int pre_tt, int post_tt)
{
 ...
 fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("4"), (int)ix, token_text(tokens[ix]));
 return add_node(node), ix;
}
...
struct op_s {
 ...
 {.tt=TT_LEFT_PARANTHESIS, .prec=100, .assoc=ASSOC_LEFT, .arity=ARITY_GROUPING},
 {.tt=TT_LEFT_SQUARE, .prec=100, .assoc=ASSOC_LEFT, .arity=ARITY_GROUPING},
 {.tt=TT_RIGHT_PARANTHESIS, .prec=100, .assoc=ASSOC_LEFT, .arity=ARITY_GROUPING_END},
 {.tt=TT_RIGHT_SQUARE, .prec=100, .assoc=ASSOC_LEFT, .arity=ARITY_GROUPING_END},
           ...
           size_t Xexpression(struct node_s *parent, struct token_s **tokens)
{
 ...
 while (!done) {
  ...
  if((op=getop(tokens[ix]->type, arity))) {
...
static size_t inline_asm(struct node_s *parent, struct token_s **tokens)
{
 ...
 fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Start"), (int)ix, token_text(tokens[ix]));
 ...
 fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Returning"), (int)ix, token_text(tokens[ix]));
...
static size_t postfix_expression(struct node_s *parent, struct token_s **tokens)
{
 ...
 if ((parsed=primary_expression(node, tokens+ix))) ix+=parsed;
  ...
  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Found left paren"), (int)ix, token_text(tokens[ix]));

  if ((parsed=type_name(node, tokens+ix))) ix+=parsed;
  else return free_node(node);

  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Parsed type name"), (int)ix, token_text(tokens[ix]));

  if (tokens[ix]->type==TT_RIGHT_PARANTHESIS) ++ix;
  else return free_node(node);

  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Found right paren"), (int)ix, token_text(tokens[ix]));

  if (tokens[ix]->type==TT_LEFT_CURLY) ++ix;
  else return free_node(node);

  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Found left curly"), (int)ix, token_text(tokens[ix]));
  ...
  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Parsed initializer list"), (int)ix, token_text(tokens[ix]));
  ...
  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Found right curly"), (int)ix, token_text(tokens[ix]));

 }

 fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Moving on to actual postfix part"), (int)ix, token_text(tokens[ix]));


 if (ix>0u) {
  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Yes we have parsed something already"), (int)ix, token_text(tokens[ix]));
  found_postfix=1;
...
static size_t cast_expression(struct node_s *parent, struct token_s **tokens)
{
 ...
 if (tokens[ix]->type==TT_LEFT_PARANTHESIS) {
  ...
  if ((parsed=type_name(node, tokens+ix))) {
   ...
   fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Gave up (type_name) cast_expression"), (int)ix, token_text(tokens[ix]));
  }
  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("No left parenthesis starting ( type_name ) cast_expression"), (int)ix, token_text(tokens[ix]));
 }

 ix=0u;
 fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Trying unary_expression"), (int)ix, token_text(tokens[ix]));

 if ((parsed=unary_expression(node, tokens+ix))) ix+=parsed;
 else return free_node(node);

 fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("After trying unary_expression"), (int)ix, token_text(tokens[ix]));
...
# 957 "parser.c"
 ...
 if ((parsed=postfix_expression(node, tokens+ix))) ix+=parsed;
  ...
  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Found ++/--"), (int)ix, token_text(tokens[ix]));
  ...
  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Parsed unary operator"), (int)ix, token_text(tokens[ix]));
  ...
  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Found sizeof"), (int)ix, token_text(tokens[ix]));
  ...
  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Found alignof"), (int)ix, token_text(tokens[ix]));
...
static size_t assignment_expression(struct node_s *parent, struct token_s **tokens)
{
 ...
 if ((parsed=unary_expression(node, tokens+ix))) {
  ...
  if (is_any_of(tokens[ix]->type, 11, (int []){
    ...
    TT_BIT_OR_ASSIGNMENT_OP})) {
   ++ix;
   fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("1a"), (int)ix, token_text(tokens[ix]));
   ...
   fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("1b"), (int)ix, token_text(tokens[ix]));

   return add_node(node), ix;
  } else ix=0u;
 }

 fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Trying conditional?"), (int)ix, token_text(tokens[ix]));

 if ((parsed=conditional_expression(node, tokens+ix))) {
  ix+=parsed;
  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Parsed conditional expression"), (int)ix, token_text(tokens[ix]));
 }
...
static size_t struct_declaration(struct node_s *parent, struct token_s **tokens)
{
 ...
 fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Parsed specifier qualifier list"), (int)ix, token_text(tokens[ix]));

 if ((parsed=separated(node, tokens+ix, STRUCT_DECLARATOR_LIST, struct_declarator, TT_COMMA_OP, 0u))) ix+=parsed;

 fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Parsed struct declarator list"), (int)ix, token_text(tokens[ix]));

 if (tokens[ix]->type==TT_SEMICOLON_OP) ++ix;
 else return free_node(node);

 fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Found semicolon"), (int)ix, token_text(tokens[ix]));
...
# 1377 "parser.c"
 ...
 while ((tt=tokens[ix]->type)==TT_LEFT_PARANTHESIS || (tt=tokens[ix]->type)==TT_LEFT_SQUARE) {
  ...
  if (tt==TT_LEFT_SQUARE) {
   ...
   if ((parsed=inline_asm(node, tokens+ix))) ix+=parsed;
  }
 }
...
static size_t initializer(struct node_s *parent, struct token_s **tokens)
{
 ...
 if ((parsed=assignment_expression(node, tokens+ix))) ix+=parsed;
  ...
  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Founde right curly"), (int)ix, token_text(tokens[ix]));
 } else return free_node(node);
...
static size_t init_declarator(struct node_s *parent, struct token_s **tokens)
{
 ...
 fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Parsed declarator"), (int)ix, token_text(tokens[ix]));

 if (tokens[ix]->type==TT_ASSIGNMENT_OP) {
  ++ix;

  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Found assignment op"), (int)ix, token_text(tokens[ix]));

  if ((parsed=initializer(node, tokens+ix))) ix+=parsed;
  else error_node_token(node, tokens[ix], "Expected initializer");

  fprintf(((__getreent())->_stderr), " >> %s (%d) >> %s >> ix=%d >> %s\n", __func__, node->level, ("Parsed initializer"), (int)ix, token_text(tokens[ix]));
 }
...
static size_t constant_expression(struct node_s *parent, struct token_s **tokens)
{
 return conditional_expression(parent, tokens);
}
